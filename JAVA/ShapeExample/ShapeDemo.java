...
public class ShapeDemo
{
	...
	/**
     *  Sorts an array of type Shape objects implementing the
     *  selection sort algorithm
     *
     *  @param pShapes    the array of Shape objects to be sorted
     *  @param numShapes  the number of Shape objects in the array
     *  @return           the sorted array
     */
    public static Shape[] sortArray(Shape[] pShapes, int numShapes) {
        boolean swapped;
        do {
            swapped = false;
            for (int i = 0; i < (numShapes-1); i++) {
                if (pShapes[i].area() > pShapes[i+1].area()) {
                    Shape tempVal = pShapes[i];
                    pShapes[i] = pShapes[i+1];
                    pShapes[i+1] = tempVal;
                    swapped = true;
                }
            }
